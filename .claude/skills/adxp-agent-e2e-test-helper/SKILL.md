...
**Prompt 파일 생성 워크플로우** (prompt_id가 있는 노드마다 반복):

JSON을 직접 요청하지 말고, 아래처럼 텍스트로 물어본다:

```
[{node_name}] 프롬프트 내용을 입력해주세요.

- 프롬프트 이름 (name):
- System Prompt (mtype=1):
- User Prompt (mtype=2):
- 태그 (tags, 쉼표 구분, 없으면 생략):
```

입력받으면:
1. messages의 `{{변수명}}` 패턴을 자동 추출하여 `variables` 배열 구성
2. 아래 Import API 형식으로 `prompt_{uuid}.json` 생성
...
```json
{
  ...
  "release": true,
  "messages": [
    { "message": "<system prompt 내용>", "mtype": 1 },
    { "message": "<user prompt 내용>", "mtype": 2 }
  ],
  "tags": [
    { "tag": "<태그1>" }
  ],
...
> **주의**: prompt JSON에는 `uuid`, `version_id`, `sequence` 필드를 포함하지 않는다.
> `release`는 항상 `true`.
> `variables`는 messages 내 `{{변수명}}` 패턴을 파싱해서 자동 생성.

tool / mcp / knowledge 파일은 `echoapi.json`에서 request body 예시를 찾아 옵션으로 제시하고, 없으면 직접 입력 요청.
